...
const DELIVERY_MONTH_LABEL = "Outubro";
...
function SimulatorContent() {
  ...
  // Parcela única: padrão 14,8% do valor final (quando não preenchido manualmente)
  const unicaVal = unicaTouched ? unicaManual : finalPropertyValue * (DEFAULT_UNICA_PERCENT / 100);
  ...
  const result: CalculationResult = useMemo(() => {
    ...
    // Parcela única: mês anterior à entrega (setembro = paymentLimit month)
  ...
  const handleUnicaInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    setUnicaTouched(true);
    const { formatted } = formatInputAsCurrency(e.target.value);
    setUnicaValueInput(formatted);
  };
  ...
  const generatePDF = useCallback(async () => {
    ...
    doc.text("Venice Park", margin, 20);
    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");
    doc.text("Simulacao Comercial - Fluxo de Pagamento", margin, 30);
    ...
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("Informacoes da Simulacao", margin, yPos);
    yPos += 10;
    autoTable(doc, {
      startY: yPos,
      head: [["Descricao", "Informacao"]],
      body: [
        ["Unidade", unitName || "Nao informado"],
        ["Area", initialArea || "—"],
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ...(result.unicaValue > 0 ? [
          ["Unica (mes anterior a entrega)", formatBRL(result.unicaValue), `${result.unicaPercent.toFixed(2)}%`, `1 parcela em ${result.unicaDate}`],
        ] : []),
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ["Saldo Final do Imovel", formatBRL(result.habiteseBalance)],
        ["Total para Quitacao", formatBRL(result.habiteseAmount)],
      ],
      theme: "striped",
    ...
    if (inccMode !== "none" && result.inccAccumulatedPercent > 0) {
      ...
      autoTable(doc, {
        ...
        head: [["Descricao", "Valor"]],
        body: [
          ["Taxa Mensal Utilizada", `${inccMonthlyRate.toFixed(3)}% ao mes`],
          ["Metrica Utilizada", inccMetricLabel],
          ["Fonte dos Dados", inccSourceLabel],
          ["Periodo de Correcao", `${constructionMonths} meses`],
          ["Correcao Acumulada", `${result.inccAccumulatedPercent.toFixed(2)}%`],
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(0, 0, 0);
    doc.text("Observacoes Importantes", margin, yPos);
    yPos += 8;
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    ...
    const notes = [
      ...
      "O saldo devedor devera ser quitado ate o habite-se ou financiado com o banco de preferencia.",
      "Importante: Os saldos devedores de todas as parcelas serao corrigidos mensalmente pelo INCC (Indice Nacional de Custo da Construcao) ate o habite-se e posteriormente por IGPM + 1%.",
      `A captacao durante as obras deve ser de no minimo ${MIN_CAPTATION_PERCENT}% do valor do imovel.`,
      "Os valores, condicoes e disponibilidade apresentados podem sofrer alteracao sem aviso previo.",
    ];
    notes.forEach((note) => {
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      doc.text(`Pagina ${i} de ${totalPages} - Venice Park`, pageWidth / 2, pageHeight - 10, { align: "center" });
    }

    const fileName = `Simulacao_Venice_Park_${(unitName || "unidade").replace(/\s+/g, "_")}_${new Date().toISOString().slice(0, 10)}.pdf`;
    try {
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-lg border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <Link href="/projetos" className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-medium flex items-center gap-2">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 mt-2 max-w-xl mx-auto">Preencha os dados abaixo e simule o plano de pagamento personalizado para o seu apartamento no Venice Park.</p>
        </div>
              ...
              <div className="p-6 space-y-5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full h-12 px-4 rounded-xl border border-slate-200 bg-slate-50 text-sm font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:bg-white transition-all placeholder:text-slate-400 text-right"
                  />
                  <p className="text-xs text-slate-500 mt-1">Padrão: 10% do valor final do imóvel</p>
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full h-12 px-4 rounded-xl border border-slate-200 bg-slate-50 text-sm font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:bg-white transition-all placeholder:text-slate-400"
                  />
                  <p className="text-xs text-slate-500 mt-1">Não é permitido selecionar datas anteriores</p>
                </div>
                <div>
              ...
              <div className="p-6 space-y-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={`Padrão: ${DEFAULT_UNICA_PERCENT}% do valor final`}
                    className="w-full h-12 px-4 rounded-xl border border-slate-200 bg-slate-50 text-sm font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:bg-white transition-all placeholder:text-slate-400 text-right"
                  />
                  <p className="text-xs text-slate-500 mt-1">Paga no mês anterior à entrega ({DELIVERY_MONTH_LABEL.toLowerCase()} de {DELIVERY_YEAR}). Padrão: {DEFAULT_UNICA_PERCENT}% do valor final. Compõe a captação da obra.</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-[#0D1B2A] to-[#0D1B2A] rounded-2xl shadow-lg p-6 text-white">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-lg font-bold">{formatBRL(propertyValue)}</p>
                </div>
                <div className="pt-4 border-t border-white/10">
                  <p className="text-white/60 text-xs mb-1">Valor com Desconto</p>
                  <p className="text-2xl font-extrabold tracking-tight">{formatBRL(result.finalPropertyValue)}</p>
                </div>
              </div>
              <div className="mt-6">
                ...
                <div className="w-full h-2.5 rounded-full bg-white/10 overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      result.captationPercent >= MIN_CAPTATION_PERCENT ? "bg-emerald-400" : result.captationPercent >= MIN_CAPTATION_PERCENT - 5 ? "bg-amber-400" : "bg-red-400"
                    }`}
                    style={{ width: `${Math.min(result.captationPercent, 100)}%` }}
                  />
                </div>
                {result.captationPercent > 0 && result.captationPercent < MIN_CAPTATION_PERCENT && (
                  <p className={`text-xs mt-2 text-right ${result.captationPercent < MIN_CAPTATION_PERCENT - 5 ? "text-red-300" : "text-amber-300"}`}>Mínimo: {MIN_CAPTATION_PERCENT}%</p>
                )}
              </div>
                  ...
                  {resultRows.map((row, i) => (
                    <div key={i} className={`rounded-xl p-4 border ${row.bold ? "bg-emerald-50 border-emerald-200" : row.isIncc ? "bg-amber-50 border-amber-200" : "bg-slate-50 border-slate-100"}`}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {row.percent && <span className={`text-xs px-2 py-0.5 rounded-full ${row.bold ? "bg-emerald-200 text-emerald-800" : "bg-slate-200 text-slate-600"}`}>{row.percent}%</span>}
                      </div>
                    ...
                    <tbody>
                      {resultRows.map((row, i) => (
                        <tr key={i} className={row.bold ? "bg-emerald-50 border-t border-emerald-200" : row.isIncc ? "border-t border-amber-200 bg-amber-50" : "border-t border-slate-100"}>
                          ...
                          <td className={`py-3 px-4 text-right ${row.bold ? "font-bold text-emerald-700" : "text-slate-500"}`}>{row.percent}%</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                    ...
                    <div className="flex flex-wrap gap-2 mb-4 bg-slate-100 p-1.5 rounded-xl">
                      {(["sinal", "mensal", "semestral", "unica", "habitese"] as const).map((tab) => (
                        <button
                          ...
                          className={`flex-1 min-w-[70px] px-3 py-2 text-xs sm:text-sm font-medium rounded-lg transition-all ${
                            activeTab === tab
                          ...
                          <table className="w-full text-sm">
                            <thead className="sticky top-0 bg-slate-50">
                              <tr className="border-b border-slate-100">
                                <th className="text-left py-2 px-4 text-xs font-semibold text-slate-500 uppercase">Parc.</th>
                                <th className="text-left py-2 px-4 text-xs font-semibold text-slate-500 uppercase">Data</th>
                                <th className="text-right py-2 px-4 text-xs font-semibold text-slate-500 uppercase">Valor</th>
                              </tr>
                            </thead>
                            <tbody>
                              {result.sinalRows.length > 0 ? result.sinalRows.map((row, i) => (
                                <tr key={i} className="border-b border-slate-50 hover:bg-slate-50">
                                  <td className="py-2 px-4 font-medium text-slate-700">{row.parcela}</td>
                                  <td className="py-2 px-4 text-slate-600">{row.data}</td>
                                  <td className="py-2 px-4 text-right font-bold text-slate-900">{row.valor}</td>
                                </tr>
                              )) : (
                                <tr><td colSpan={3} className="py-4 text-center text-slate-400">Nenhum dado</td></tr>
                              )}
                            </tbody>
                          </table>
                        </div>
                          ...
                          <table className="w-full text-sm">
                            ...
                            <tfoot>
                              <tr className="bg-slate-50 font-bold">
                                <td className="py-2 px-4" colSpan={2}>Total parcela única</td>
                                <td className="py-2 px-4 text-right">{formatBRL(result.unicaValue)}</td>
                              </tr>
                            </tfoot>
                          </table>
                        </div>
                    ...
                    {activeTab === "habitese" && (
                      <div className="p-4 space-y-4 bg-slate-50 rounded-xl">
                        <div className="p-4 rounded-xl bg-white border border-slate-200">
                          ...
                          <p className="text-sm text-slate-500 mt-1">Saldo para financiamento bancário — pode ser quitado ou financiado com instituição de preferência</p>
                        </div>
                        {inccMode !== "none" && result.inccAccumulatedPercent > 0 && (
                        ...
                        <div className="space-y-2">
                          <h5 className="font-semibold text-slate-800 text-xs uppercase tracking-wider">Composição do Financiamento</h5>
                          <div className="p-3 rounded-lg bg-white border border-slate-100">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-sm font-bold text-slate-900">{formatBRL(inccMode !== "none" && result.inccAccumulatedPercent > 0 ? result.mRemainingCorrected : result.monthlyRemaining)}</span>
                            </div>
                          </div>
                          <div className="p-3 rounded-lg bg-white border border-slate-100">
                            <div className="flex items-center justify-between">
                              <span className="text-sm text-slate-600">Parcelas semestrais restantes</span>
                              <span className="text-sm font-bold text-slate-900">{formatBRL(inccMode !== "none" && result.inccAccumulatedPercent > 0 ? result.sRemainingCorrected : result.semesterRemaining)}</span>
                            </div>
                          </div>
                          <div className="p-3 rounded-lg bg-white border border-slate-100">
                            <div className="flex items-center justify-between">
                              <span className="text-sm text-slate-600">Saldo final do imóvel</span>
                              <span className="text-sm font-bold text-slate-900">{formatBRL(inccMode !== "none" && result.inccAccumulatedPercent > 0 ? result.hBalanceCorrected : result.habiteseBalance)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
...
export default function SimuladorVeniceParkPage() {
  return (
    <Suspense fallback={
