...
function SimulatorContent() {
  ...
  useEffect(() => {
    if (propertyValue > 0) {
      setShowResults(true);
      track({ event_type: "simulador_calculate", resource_type: "empreendimento", metadata: { empreendimento: "moment", unidade: unitName, valor_imovel: result.finalPropertyValue, captacao_percent: result.captationPercent } });
    }
  }, [result]);
  ...
  const generatePDF = useCallback(async () => {
    track({ event_type: "simulador_export_pdf", resource_type: "empreendimento", metadata: { empreendimento: "moment", unidade: unitName } });
  ...
  const resultRows = useMemo(() => [
    ...
    { description: "Taxa de Decoração", value: formatBRL(DECORATION_FEE), percent: null as number | null, note: `${result.decorationInstallments}x de abr/2027 a jan/2028`, bold: false, isHighlight: false, isIncc: false },
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium hidden sm:block">Simulador Moment</p>
              </div>
            </div>
            <Link href="/moment" className="text-sm text-slate-600 hover:text-slate-900 transition-colors font-medium flex items-center gap-2">
              <span className="hidden sm:inline">← Voltar ao Moment</span>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              ...
              <div className="p-6 space-y-5">
                ...
                <div>
                  ...
                  <input type="text" value={unitName} onChange={(e) => setUnitName(e.target.value)} placeholder="Ex: Apartamento 101" className="w-full h-12 px-4 rounded-xl border border-slate-200 bg-slate-50 text-sm font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:bg-white transition-all placeholder:text-slate-400" />
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6 lg:col-span-2 lg:sticky lg:top-24 self-start">
            ...
            <div className="bg-gradient-to-br from-[#0D1B2A] to-[#0D1B2A] rounded-2xl shadow-lg p-6 text-white">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs bg-white/10 px-2 py-1 rounded-full">Entrega: Fev/{DELIVERY_YEAR}</span>
              </div>
