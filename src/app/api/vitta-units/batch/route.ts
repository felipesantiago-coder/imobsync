...
/**
 * PATCH em lote das unidades Vitta (tabela `vitta_units`).
 ...
 * Corpo: { status, unidades: [{ bloco, unidade, andar }] } — `andar` opcional,
 * usado como desambiguação quando informado (paridade c/ PATCH individual).
 */
export async function PATCH(request: NextRequest) {
  try {
    ...
    const result = await applyBatchStatusUpdate({
      ...
      resolveColumns: "id, status, unidade, bloco, andar",
      empreendimentoRef: "vitta",
    ...
    console.error("Erro no PATCH /api/vitta-units/batch:", err);
