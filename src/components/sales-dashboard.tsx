...
const ExpandedCard = memo(function ExpandedCard({ unit, onClose, closing = false, onAnimEnd }: { unit: Unit; onClose: () => void; closing?: boolean; onAnimEnd: (e: React.AnimationEvent) => void }) {
  const colors = typeColors[unit.tipoArea];
  const status = statusLabels[unit.status];
...
export default function SalesDashboard({ isAdmin = false, isCoordinator = false, hideHeader = false, initialUnits = null }: { isAdmin?: boolean; isCoordinator?: boolean; hideHeader?: boolean; initialUnits?: Record<string, unknown>[] | null }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100 flex flex-col">
      ...
      {/* Expanded centered card overlay — presença CSS (exit animado + desmontagem) */}
