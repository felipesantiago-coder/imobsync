...
import { Building2, Car, Maximize2, DollarSign, ChevronUp, Filter, X, BedDouble, Check, LogOut, Sun, Calculator, Pencil, ArrowLeft, Radio, ListChecks } from "lucide-react";
...
const ExpandedCard = memo(function ExpandedCard({ unit, onClose, closing = false, onAnimEnd }: { unit: MomentUnit; onClose: () => void; closing?: boolean; onAnimEnd: (e: React.AnimationEvent) => void }) {
  const colors = typeColors[unit.tipologia];
  const status = statusLabels[unit.status];
...
const FloorSection = memo(function FloorSection({
  ...
  onToggleSelect?: (unit: MomentUnit) => void;
  onToggleFloorSelect?: (units: MomentUnit[]) => void;
}) {
...
export default function MomentDashboard({ isAdmin = false, isCoordinator = false, hideHeader = false, initialUnits = null }: { isAdmin?: boolean; isCoordinator?: boolean; hideHeader?: boolean; initialUnits?: Record<string, unknown>[] | null }) {
  ...
  const expandedPresence = useCssPresence<MomentUnit | null>(selectedUnit, "imsOverlayOut");
  ...
  const handleBatchToggleMany = useCallback((list: MomentUnit[]) => {
    setBatchFeedback(null);
  ...
  const handleBatchClear = useCallback(() => { setBatchFeedback(null); setSelectedForBatch(new Set()); }, []);
  ...
  const confirmBatchStatusChange = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch("/api/moment-units/batch", {
        method: "PATCH",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      setUnits((prev) => prev.map((u) => (okKeys.has(String(u.unidade)) ? { ...u, status: newStatus } : u)));
