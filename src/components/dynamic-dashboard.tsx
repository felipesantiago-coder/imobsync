...
export default function DynamicDashboard({
  ...
  const confirmBatchStatusChange = useCallback(async () => {
    ...
    setBatchFeedback(null);
    try {
      const selecionadas = Array.from(selectedForBatch)
        .map((unitId) => units.find((u) => u.id === unitId))
        .filter((u): u is ProjetoUnit => Boolean(u))
        .map((u) => ({ unidade: u.unidade, bloco: u.bloco || undefined }));
      ...
      const res = await fetch(
        `/api/admin-sistema/empreendimentos/${empreendimentoId}/units/batch`,
        {
          method: "PATCH",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ status: newStatus, unidades: selecionadas }),
        }
      );
      if (!res.ok) {
      ...
      const okIds = new Set((data.updated ?? []).map((r) => r.id));
      setUnits((prev) =>
        prev.map((u) => (okIds.has(u.id) ? { ...u, status: newStatus } : u))
      );
      // Unidades com falha permanecem selecionadas para nova tentativa
      setSelectedForBatch((prev) => {
        const remaining = new Set<string>();
        for (const unitId of prev) if (!okIds.has(unitId)) remaining.add(unitId);
        return remaining;
      });
      ...
      if (failedCount > 0) {
        setBatchFeedback(
          failedCount >= data.total
            ...
            : { message: `${okIds.size} de ${data.total} atualizadas — ${failedCount} não puderam ser atualizadas e continuam selecionadas.`, tone: "warning" }
        );
      }
    } catch (err) {
